...
1. 下载安装 `jdk`，并配置环境变量
   ...
   ![jdk_java_home](./imgs/tomcat/tomcat_jdk_env_java_home.png)

   ![jdk_path](./imgs/tomcat/tomcat_jdk_env_path.png)

    3. 验证环境变量

   ![java_version](./imgs/tomcat/tomcat_jdk_version.png)

2. 下载 `tomcat` ,一般考虑下载解压版
    1. `tomcat` 下载地址 https://tomcat.apache.org/
    2. 解压 `tomcat` 到非中文无空格目录
      
   ![tomcat解压目录](./imgs/tomcat/tomcat_un_zip_dir.png)

3. 启动 `tomcat` 执行 `bin/startup.bat` 

![tomcat startup](./imgs/tomcat/tomcat_startup.png)
...
![tomcat根目录](./imgs/tomcat/tomcat_un_zip_dir.png)
...
![webapps](./imgs/tomcat/tomcat_dir_webapps_default.png)
